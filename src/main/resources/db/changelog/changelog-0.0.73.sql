...
CREATE TABLE enq_location (
  ...
  feasibility_update VARCHAR(1) DEFAULT NULL CHECK (feasibility_update IN ('', '0', '1')),
  -- above is enum('','0','1') DEFAULT NULL,
  speed varchar(45) DEFAULT NULL,
  unit varchar(10) DEFAULT 'Mbps' CHECK(unit IN ('Kbps', 'Mbps', 'Gbps')),
  -- above is enum('Kbps','Mbps','Gbps') DEFAULT 'Mbps',
  connection_type varchar(10) DEFAULT NULL CHECK(connection_type IN ('Fiber', 'Wireless', 'Leased Line')),
  -- above is enum('Home','Others','FUP','Unlimited') DEFAULT NULL,
  validity varchar(45) DEFAULT NULL,
  refeasibility_check varchar(10) DEFAULT NULL, CHECK(refeasibility_check IN ('', '0', '1')),
  -- above is enum('0','1') DEFAULT NULL,
...
CREATE TABLE enquiry_management (
  ...
  payment_type varchar(10) DEFAULT 'postpaid' CHECK(payment_type IN ('prepaid', 'postpaid')),
  -- above is enum('prepaid','postpaid') DEFAULT 'postpaid',
  gstinval varchar(45) DEFAULT NULL,
  created_by int DEFAULT NULL,
  CreatedOn timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  feasibility_status varchar(10) DEFAULT 'Enquiry' CHECK(feasibility_status IN ('Enquiry','Feasibile','Not_Feasibile','On_Hold','Quotation_Given','Under_Negotiation','Order_Confirmed','Service_Provisioning_Completed')),
  ...
  assigning_status varchar(10) NOT NULL DEFAULT 0 CHECK(assigning_status IN ('0','1')),
  --above is enum('0','1') DEFAULT 0,
  quotation_created varchar(10) DEFAULT 0 CHECK(quotation_created IN ('0','1')),
  -- above is enum('0','1') DEFAULT 0,
  -- Primary key constraint
