...
class CafDetailsControllerTest {
    ...
    @MockBean
    private CafDetailsService service;

    @Autowired
    private CafDetailsController controller;
    ...
    void testDetailsFetchAll() throws Exception {
        ...
        mockMvc.perform(get("/api/caf/details/fetch-all")
    ...
    void testCaf_detailById() throws Exception {
        ...
        mockMvc.perform(get("/api/caf/detail/{id}", id)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Fetched"))
