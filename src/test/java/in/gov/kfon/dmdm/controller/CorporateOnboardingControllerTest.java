...
class CorporateOnboardingControllerTest {
  ...
  @Test
  void testServicesListFetchAll() throws Exception {
    when(service.servicesListFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/services-list/fetch-all").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  }
  ...
  @Test
  void testServicesListById() throws Exception {
    when(service.servicesListFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(
            get("/api/corporate/service-list/{id}", id).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  }
  ...
  @Test
  void testSubCustomersFetchAll() throws Exception {
    when(service.subCustomersFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/sub-customers/fetch-all").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  }
  ...
  @Test
  void testSubCustomerById() throws Exception {
    when(service.subCustomersFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(
            get("/api/corporate/sub-customer/{id}", id).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  }
  ...
  @Test
  void testSubPackageRenewalHistoryFetchAll() throws Exception {
    when(service.subPackageRenewalHistoryFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/sub-package/renewal-histories/fetch-all")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  ...
  @Test
  void testSubPackageRenewalHistoryById() throws Exception {
    when(service.subPackageRenewalHistoryFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(
            get("/api/corporate/sub-package/renewal-history/{id}", id)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  ...
  @Test
  void testSubPackageFetchAll() throws Exception {
    when(service.subPackageFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/sub-packages/fetch-all").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  }
  ...
  @Test
  void testSubPackageById() throws Exception {
    when(service.subPackageFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(get("/api/corporate/sub-package/{id}", id).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  }
  ...
  @Test
  void testSubServiceListsFetchAll() throws Exception {
    when(service.subServiceListsFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/sub-service/lists/fetch-all")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  ...
  void testListById() throws Exception {
    when(service.subServiceListsFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(
            get("/api/corporate/sub-service/list/{id}", id).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  }
  ...
  @Test
  void testSubFinancesFetchAll() throws Exception {
    when(service.subFinancesFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/sub-finances/fetch-all").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  }
  ...
  @Test
  void testSubFinanceById() throws Exception {
    when(service.subFinancesFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(get("/api/corporate/sub-finance/{id}", id).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  }
  ...
  @Test
  void testSubscriberDetailsFetchAll() throws Exception {
    when(service.subscriberDetailsFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/subscriber-Details/fetch-all")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  ...
  @Test
  void testSubscriberDetailById() throws Exception {
    when(service.subscriberDetailsFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(
            get("/api/corporate/subscriber-Detail/{id}", id)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  ...
  @Test
  void testSubOnlineRechargesFetchAll() throws Exception {
    when(service.subOnlineRechargesFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/sub-online/recharges/fetch-all")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  ...
  @Test
  void testRechargeById() throws Exception {
    when(service.subOnlineRechargesFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(
            get("/api/corporate/sub-online/recharge/{id}", id)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  ...
  @Test
  void testSubScribersDocumentFetchAll() throws Exception {
    when(service.subScribersDocumentFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/subscribers-documents/fetch-all")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  ...
  @Test
  void testSubscribersDocumentById() throws Exception {
    when(service.subScribersDocumentFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(
            get("/api/corporate/subscribers-document/{id}", id)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  ...
  @Test
  void testSubscribersFetchAll() throws Exception {
    when(service.subscribersFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/subscribers/fetch-all").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  }
  ...
  @Test
  void testSubscriberById() throws Exception {
    when(service.subscribersFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(get("/api/corporate/subscriber/{id}", id).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  }
  ...
  @Test
  void testSubShiftDetailsFetchAll() throws Exception {
    when(service.subShiftDetailsFetchAll()).thenReturn(List.of(lookup));
  ...
  @Test
  void testSubshiftDetailById() throws Exception {
    when(service.subShiftDetailsFetchById(any(UUID.class))).thenReturn(lookup);
  ...
  @Test
  void testSupportSubdocumentsFetchAll() throws Exception {
    when(service.supportSubdocumentsFetchAll()).thenReturn(List.of(lookup));

    mockMvc
        .perform(
            get("/api/corporate/support-sub/documents/fetch-all")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data[0].code").value("CORP001"));
  ...
  @Test
  void testDocumentById() throws Exception {
    when(service.supportSubdocumentsFetchById(any(UUID.class))).thenReturn(lookup);

    mockMvc
        .perform(
            get("/api/corporate/support-sub/document/{id}", id)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message").value("Fetched"))
        .andExpect(jsonPath("$.data.code").value("CORP001"));
  ...
  @Test
  void testWorkOrderFetchAll() throws Exception {
